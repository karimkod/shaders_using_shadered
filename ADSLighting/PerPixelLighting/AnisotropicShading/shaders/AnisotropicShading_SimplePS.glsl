...
void main(){
	...
	vec3 normalizedECtangent = normalize(ECtangent);
	
	float de = dot(normalizedECtangent, viewRay);
	float dl = dot(normalizedECtangent, lightRay);
	


	vec3 ambient = vec3(ambientColor * colorMaterial * aCoeff); 
	vec3 diffuse = vec3(lightColor* colorMaterial * dCoeff * max(0, dot(lightRay, ECnormal))) ;
	vec3 specular = vec3(0);
	float specComp = de*dl + sqrt(1-dl*dl)*sqrt(1-de*de);
	
	if(specComp > 0)
	
	{
	specular = vec3(lightColor*colorMaterial*sCoeff * pow(specComp, materialShininess));
	}
